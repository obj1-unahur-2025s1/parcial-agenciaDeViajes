class Pack {
    ...
    method costoFinal() = 
        precioBase + beneficios.filter({unBeneficio => unBeneficio.estaVigente()}).sum({beneficioVigente => beneficioVigente.costo()})
...
class PackNacional inherits Pack {
    ...
    override method esPremium() =
        duracionEnDias > 10 && coordinador.esAltamenteCalificado()
}
...
class PackInternacional inherits Pack {
    ...
    override method esPremium() =
        esDeInteres && duracionEnDias > 20 && cantidadDeEscalas == 0
}
...
class Coordinador {
    ...
    method rol(unRol) {
        if(not rolesValidos.contains(unRol)) {
            throw new Exception(message= "El rol debe ser guia, asistente logístico o acompañante") // self.error("")
        }
        rol = unRol
    }

    method esAltamenteCalificado() = cantidadDeViajes > 20 && rol.condicionAdicional(self)
}
...
class PackProvincial inherits PackNacional {
    var property cantidadDeCiudades
     ...
     override method esPremium() =
        actividades.size() >= 4 &&
        cantidadDeCiudades > 5 &&
        beneficios.count({unBeneficio => unBeneficio.estaVigente()}) >= 3

    override method costoFinal() =
        if(self.esPremium()) super() * 1.05 else super()
}